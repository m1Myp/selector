...
```
Добавить утильс и в стейдж 2 файлик явы для дфр парсера
...
- `.???` — для поддержки новых форматов, реализуйте `load_profile()`
...
После выполнения задачи скрипт генерирует файл с весами для выбранных юнит-тестов `$WORK_DIR/stages/weight.json`. TODO итоговую похожесть
...
TODO идентификаторы
В виде матрицы оформить

- $` S = \{s_1, s_2, ..., s_n\} `$ — это набор всех встречающихся идентификаторов. Каждый элемент гистограммы теста $` i `$ ($` s_i `$) — это вектор чисел $` s_i = [s_{i1}, s_{i2}, ..., s_{in}] `$ с вещественными значениями $` s_{ij} \in \mathbb{R} `$.
...
TODO Секция детали. Раскрыть хотнесс компрешн и блок компрешн
TODO Юнит тесты или сэмплы